    ...
    while (getline(cin, str))    {
        ...
        for (int i = 0; i < input.size(); i++) {
            for (int j = 0; j < input[0].size(); j++) {
                if (input[i][j] == 'W') {
                    int w_index = getIndex(i, j, input[0].length());
                    weighted.push_back(make_pair(1, make_pair(source, w_index)));
                    int new_w_index = nodes++;
                    weighted.push_back(make_pair(1, make_pair(w_index, new_w_index)));
                    ...
                    for (int k = 0; k < good_neigh.size(); k++) {
                        int neigh_index = getIndex(good_neigh[k].first,
                                                   ...
                                                   input[0].length());
                        weighted.push_back(
                                make_pair(1, make_pair(new_w_index, neigh_index)));
                    }
                }
                if (input[i][j] == 'I') {
                    int i_index = getIndex(i, j, input[0].length());
                    int new_i_index = nodes++;
                    weighted.push_back(make_pair(1, make_pair(i_index, new_i_index)));
                    vector<pair<int, int>> good_neigh = getCharNeighbours(input,
                                                                          'N',
                                                                          i, j);
                    for (int k = 0; k < good_neigh.size(); k++) {
                        int neigh_index = getIndex(good_neigh[k].first,
                                                   good_neigh[k].second,
                                                   input[0].length());
                        weighted.push_back(make_pair(1, make_pair(new_i_index, neigh_index)));
                    }
                }
                if (input[i][j] == 'N') {
                    int n_index = getIndex(i, j, input[0].length());
                    int new_n_index = nodes++;
                    weighted.push_back(make_pair(1, make_pair(n_index, new_n_index)));
                    weighted.push_back(make_pair(1, make_pair(new_n_index, target)));
                }
            }
        }
